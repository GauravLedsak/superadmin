...
function StoreProvider({ children }) {
  ...
  const api = {
    ...
    setTenantStatus: (id, status) => { setClients((cs) => cs.map((c) => (c.id === id ? { ...c, status } : c))); notify(`Client ${status === "Suspended" ? "suspended" : "reactivated"}`); },
...
function QueuesPage() {
  ...
  const [queues, setQueues] = useState([
    { name: "lead-ingestion", pending: 8, processing: 3, failed: 0, rate: "1.2k/min", status: "Healthy" },
    { name: "ai-enrichment", pending: 142, processing: 12, failed: 4, rate: "80/min", status: "Backed up" },
    { name: "notifications", pending: 24, processing: 6, failed: 1, rate: "600/min", status: "Healthy" },
    { name: "webhooks-out", pending: 2, processing: 1, failed: 0, rate: "300/min", status: "Healthy" },
    { name: "billing-jobs", pending: 0, processing: 0, failed: 0, rate: "idle", status: "Idle" },
  ]);
  const st = { Healthy: "success", "Backed up": "warning", Idle: "gray" };
  const retry = (name) => { setQueues((qs) => qs.map((q) => q.name === name ? { ...q, failed: 0, status: "Healthy" } : q)); store.notify(`Retried failed jobs in ${name}`); };
  return (
    <>
      <PageHeader title="Queue Monitor" desc="Background jobs, throughput, retries and dead-letter queues" actions={<Button onClick={() => store.notify("Queues refreshed")}><RefreshCw size={15} /> Refresh</Button>} />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <Kpi label="Jobs Pending" value={String(queues.reduce((s, q) => s + q.pending, 0))} sub="across queues" trend="warn" />
        <Kpi label="Processing" value={String(queues.reduce((s, q) => s + q.processing, 0))} sub="live workers" />
        <Kpi label="Failed (24h)" value={String(queues.reduce((s, q) => s + q.failed, 0))} sub="dead-letter" trend="neg" />
        <Kpi label="Throughput" value="2.4k/min" sub="peak today" trend="pos" />
      </div>
      <Card>
        <Table head={["Queue", "Pending", "Processing", "Failed", "Rate", "Status", ""]}>
          {queues.map((q, i) => (
            <tr key={i} className="hover:bg-[#F8F9FC]">
              <Td className="font-mono text-xs font-medium">{q.name}</Td>
              <Td>{q.pending}</Td><Td>{q.processing}</Td>
              <Td style={{ color: q.failed ? T.danger : T.text }}>{q.failed}</Td>
              <Td className="text-xs" style={{ color: T.text2 }}>{q.rate}</Td>
              <Td><Badge tone={st[q.status]}>{q.status}</Badge></Td>
              <Td>{q.failed > 0 && <Button size="sm" onClick={() => retry(q.name)}><RefreshCw size={13} /> Retry</Button>}</Td>
            </tr>
          ))}
        </Table>
      </Card>
...
function AutomationPage() {
  ...
  return (<>
    <PageHeader title="Automation Center" desc="Workflows, triggers and lead-routing rules" actions={<Button variant="primary" onClick={() => store.notify("New workflow created")}><Plus size={15} /> New Workflow</Button>} />
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
